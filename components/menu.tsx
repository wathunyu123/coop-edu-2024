      ...
      <nav className="flex items-center justify-between font-bold px-5 text-white">
        {menuItems.map((item) => (
          ...
          >
            <Link
              href={item.href}
              aria-label={item.label} // เพิ่ม aria-label
              className={`flex items-center space-x-2 hover:bg-white hover:text-black rounded-xl px-4 py-2 transition-all ${
                isActive(item.activePaths) ? "bg-white text-black" : ""
              }`}
            >
              <span>{item.icon}</span>
